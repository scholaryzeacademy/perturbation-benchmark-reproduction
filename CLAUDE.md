...
# on either dataset are still untested, and a real limitation was found for
# combo conditions specifically (some gene pairs have zero control cells
# expressing both genes, which crashes with no per-condition recovery in
# run_geneformer.py's current loop) -- see Working conventions below before
# attempting a full Norman production run:

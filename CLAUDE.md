...
# Stage 5 -- metric calibration framework (BUILD_PLAN.md Sec.6 Stage 5): the
# project's key differentiating contribution. Computes an "interpolated
# duplicate" positive control (repeated random 50/50 splits of each real
# test condition's own held-out cells -- see calibration.py's docstring for
# why, and CLAUDE.md's Working Conventions below for the real numbers) and
# calibrates every Stage 4 model's score against the gap between that
# positive control and the no-change negative control. Single env
# (perturb-bench-gears-gpu -- needs `gears` for load_perturbation_data, same
# as run_baselines_gears.py; does NOT need scgpt-specific packages, it only
# reads scGPT's already-computed Stage 4 CSV as data). Run for real on both
# datasets 2026-07-31 -- see Working Conventions below:
...
├── stage5_calibration/       # calibration.py + run_calibration.py + combine_calibration.py created
...
└── test_stage5_calibration.py     # created — toy-data unit tests (positive control finiteness, pooled-aggregation arithmetic hand-verified, zero-denominator guard)
...
- **Stage 4 (conventional metrics) is implemented and has been run for real on both datasets** (2026-07-30): `src/stage4_conventional_metrics/metrics.py` implements MSE@20DEG (top-20 DE genes per condition via `scanpy.tl.rank_genes_groups`, t-test, `rankby_abs=True` — matching GEARS's own `rank_genes_groups_by_cov` convention, see the module's own docstring for why an exact match to GEARS's self-reported number isn't expected) and Pearson(delta control) (correlation of predicted vs. real expression change from control, across all genes), behind one `evaluate_model(predict_fn, adata, test_conditions, model_name)` function every model runs through identically. Two driver scripts exist because baselines/GEARS and scGPT need incompatible `cell-gears` versions (see Environment setup): `run_baselines_gears.py` (env: `perturb-bench-gears-gpu`) fits the three Stage 2 baselines fresh and loads GEARS from its existing real checkpoints (`data/gears_{dataset}_real_ckpt`, no retraining); `run_scgpt.py` (env: `perturb-bench-scgpt`) loads the fine-tuned scGPT checkpoint (`save/dev_perturb_{dataset}-*/best_model.pt`, auto-picks the most recent one that actually has a checkpoint, skipping earlier aborted attempts) and reuses `train_scgpt.py`'s own `predict()` function unchanged, wrapped to match the uniform contract. `combine_results.py` merges both drivers' per-dataset CSVs into one summary.
  ...
  **Real results** (mean per model per dataset, `n` = number of real held-out test conditions actually evaluated):
  ...
  A genuinely interesting, dataset-dependent finding (exactly the kind of thing this project exists to surface honestly, per BUILD_PLAN.md §0–§2's framing): **scGPT clearly wins on Adamson** (single-gene perturbations) on both metrics. Norman (combinatorial perturbations) doesn't have one clean winner — it splits by metric: **GEARS has the best (lowest) raw MSE@20DEG** (0.1689, beating ridge's 0.2308), but **ridge has the best (highest) Pearson(Δcontrol)** (0.6732, beating GEARS's 0.5831). **Correction (found 2026-07-31 while cross-checking these numbers during Stage 5 calibration work)**: an earlier version of this section claimed "ridge... wins on Norman on both metrics, beating both DL models" — that was wrong on MSE@20DEG specifically (GEARS's raw MSE is actually the lowest of all five Norman models, including ridge); it only holds for Pearson(Δcontrol). Left here as a reminder to actually re-derive claims from the CSVs rather than trust a prior paragraph's summary, even one written earlier in this same file. Don't average the two datasets together into one number when reporting, and don't collapse Norman's own metric-dependent split into a single "winner" either.
...
- **Stage 5 (metric calibration framework) is implemented and has been run for real on both datasets** (2026-07-31): `src/stage5_calibration/calibration.py` implements the "interpolated duplicate" positive control and the calibrated-fraction scoring; `run_calibration.py` (env: `perturb-bench-gears-gpu`) drives it per dataset, reading Stage 4's own per-condition CSVs rather than recomputing model predictions; `combine_calibration.py` merges both datasets into `calibrated_summary.csv`. See `docs/BUILD_PLAN.md` §6 Stage 5 for the framing (this is the project's key differentiating contribution) and `calibration.py`'s own docstring for the full methodology.

  **Two scope decisions were confirmed with the user before implementation** (BUILD_PLAN.md explicitly calls for statistical sign-off on this stage, given its centrality to the project's credibility):
  1. **No true biological-replicate/batch column exists in this project's real data** — confirmed directly against `adata.obs` for both datasets: only `condition`, `cell_type`, `dose_val`, `control`, `condition_name`, no batch/lane/replicate field. So the positive control is built from **repeated random 50/50 splits of each real test condition's own held-out cells into two pseudo-replicate halves** (10 splits x2 directions = 20 samples per condition, averaged; `stage5_calibration.n_splits`/`seed` in the config), not genuine separate-batch replicates — an explicit narrowing of BUILD_PLAN.md's own "held-out biological replicates" phrasing, not a silent substitution. Verified feasible beforehand: every real test condition in both datasets has >=49 cells, so no condition needed to be skipped for lack of cells to split (unlike Stage 3c's zero-co-occurring-cells limitation) — the `n_obs<2` guard in `compute_positive_control` exists for a different, real reason instead (see finding 2 below).
  2. **`no_change`'s Pearson(delta control) is mathematically NaN** (already documented under Stage 4 above). `calibrate_dataset` uses **0.0** as the negative-control Pearson reference instead (the principled "no linear relationship" stand-in for a constant/zero-variance predictor) — a documented substitution, not a silent default. `calibrated_mse_fraction` doesn't need this workaround: MSE@20DEG's negative control is no_change's own real (non-NaN) score, so `no_change`'s own calibrated_mse_fraction comes out **exactly 0.0** on both datasets when compared against itself — the pipeline sanity check this project's conventions call for, confirmed for real (not just in the unit tests). `calibrated_pearson_fraction` has no equivalent sanity-check value for `no_change` itself (its own mean Pearson is NaN, so its own calibrated_pearson_fraction is also NaN, correctly, by construction) — expected, not a bug.
  3. **Aggregation is pooled, not per-condition-ratio-then-averaged**: each calibrated fraction is computed once per (dataset, model) from three already-averaged numbers (mean MSE/Pearson across that model's own conditions, for model/negative-control/positive-control) — avoids a per-condition ratio blowing up when one weak-true-effect condition's positive control barely beats no-change for that condition alone.

  **A real, previously-undocumented finding surfaced building this** (not a Stage 5 bug — `compute_positive_control`'s `n_obs<2` guard handled it correctly, exactly as designed for an unanticipated edge case): `AMIGO3+ctrl` (Adamson) and 4 conditions (`LYL1+IER5L`, `RHOXF2BB+SET`, `RHOXF2BB+ZBTB25`, `RHOXF2BB+ctrl`, Norman) have **zero real cells** in `pert_data.adata` when loaded via `perturb-bench-gears-gpu`'s `cell-gears==0.1.2`, yet all 5 appear with real, non-degenerate scores in `run_scgpt.py`'s Stage 4 output (computed via `perturb-bench-scgpt`'s `cell-gears==0.0.2`). Confirmed this is real GEARS-library behavior, not a data problem: the raw `perturb_processed.h5ad` genuinely has cells for these conditions (e.g. 616 for `AMIGO3+ctrl`), but `PertData.load()`'s own startup log prints `"These perturbations are not in the GO graph and their perturbation can thus not be predicted"` for exactly this condition list, and `cell-gears==0.1.2` drops those conditions' cells from `pert_data.adata` entirely at load time (confirmed directly: 65899 cells in `pert_data.adata` vs. 68603 in the raw h5ad for Adamson) — `cell-gears==0.0.2` (scGPT's env) evidently does not do this filtering. **This is a materially deeper cross-version difference than Stage 4's existing note** ("the two library versions produce different real train/test splits from the same seed") already captures — it's not just a different split *assignment*, it's different *cell content available in `.adata` at all*. Handled gracefully: these conditions are dropped from the positive control (and from the freshly-recomputed no_change) with a printed `WARNING`, and `calibrate_dataset` in turn excludes them from scGPT's calibrated score specifically (also printed) rather than crashing or silently producing a wrong number — 1/22 conditions excluded for Adamson, 4/116 for Norman. scGPT's own *raw* Stage 4 metrics for these conditions are unaffected (they were computed in scGPT's own env against its own real cells, which do exist there).

  **A real documentation error in this same file was found and fixed while cross-checking these numbers** (2026-07-31): the Stage 4 write-up above previously claimed "ridge... wins on Norman on both metrics, beating both DL models (GEARS and scGPT)" — false for MSE@20DEG specifically (GEARS's raw Norman MSE, 0.1689, is actually the lowest/best of all five models, including ridge's 0.2308; ridge only wins on Pearson). Corrected in place above, with a note not to trust a prior paragraph's summary over the actual CSV.

  **Real results** (`data/stage5_results/calibrated_summary.csv`, `n_conditions` = conditions actually used for that model's calibrated score, after excluding any missing negative/positive-control coverage per the finding above):

  | dataset | model | calibrated MSE@20DEG fraction | calibrated Pearson(Δcontrol) fraction | n_conditions |
  |---|---|---|---|---|
  ...
  Calibration **preserves** Stage 4's headline finding rather than overturning it: scGPT still clearly leads Adamson on the calibrated MSE fraction (0.572 vs. ~0.35 for the other four), and its calibrated Pearson fraction is statistically indistinguishable from mean/ridge's (0.671 vs. 0.674) rather than clearly ahead as the raw Pearson numbers suggested — a real, mild recalibration nuance worth knowing but not a ranking flip. Norman's own metric-dependent split (see the corrected Stage 4 finding above) also survives calibration essentially unchanged: GEARS leads on the calibrated MSE fraction (0.621), ridge leads on the calibrated Pearson fraction (0.748) — calibration doesn't resolve which metric "should" be trusted, which is exactly the honest, unresolved-tension result this stage is meant to surface, not a shortcoming of the implementation.

  All per-dataset CSVs (`data/stage5_results/{adamson,norman}_{positive_control,calibrated}.csv`) and the merged summary (`calibrated_summary.csv`) are saved under `data/stage5_results/` (gitignored, like the rest of `data/`).
